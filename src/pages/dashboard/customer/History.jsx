...
const History = () => {
  ...
  // Modifikasi fungsi fetchPickups untuk mengambil data lengkap
  useEffect(() => {
  ...
  return (
    <div className="px-4 py-4 mx-auto max-w-7xl sm:px-6 sm:py-6">
      ...
      {/* Pickup List with Collapsible Cards */}
      {loading ? (
        ...
        <div className="space-y-4">
          {filteredPickups.map((pickup) => (
            <div key={pickup.id} className="overflow-hidden transition-shadow bg-white border border-gray-100 shadow-lg rounded-xl hover:shadow-xl">
              {/* Card Header - Always visible */}
              <div 
                className="p-4 transition-colors cursor-pointer sm:p-6 hover:bg-gray-50"
                onClick={() => toggleCard(pickup.id)}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center flex-1 gap-3">
                    <div className="flex items-center gap-2">
                      <CalendarDays className="flex-shrink-0 w-4 h-4 sm:w-5 sm:h-5 text-emerald-600" />
                    ...
                    <span className={`inline-flex px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium capitalize
                      ...
                      {pickup.status === 'completed' ? 'Selesai' :
                       ...
                       pickup.status === 'assigned' ? 'Diproses' :
                       pickup.status}
                    </span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="hidden text-right sm:block">
                      ...
                      <p className="font-bold text-gray-900">Rp {pickup.totalValue?.toLocaleString() || 0}</p>
                    </div>
                    <ChevronsUpDown className={`w-5 h-5 text-gray-400 transition-transform duration-200
                      ${expandedCards[pickup.id] ? 'rotate-180' : ''}`} 
                    />
                  </div>
                </div>
              </div>
              ...
              <div className={`transition-all duration-200 ease-in-out ${
                expandedCards[pickup.id] ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
              } overflow-hidden`}>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    {pickup.location && (
                      <div className="flex items-start gap-2 p-3 rounded-lg bg-gray-50">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium sm:text-base">{pickup.location}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {pickup.notes && (
                      <div className="flex items-start gap-2 p-3 rounded-lg bg-gray-50">
                        ...
                        <div className="text-xs text-gray-600 sm:text-sm">{pickup.notes}</div>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="py-12 text-center bg-white shadow-lg sm:py-16 rounded-xl">
          ...
          <h3 className="mb-2 text-lg font-medium text-gray-900 sm:text-xl">Tidak ada riwayat pickup</h3>
